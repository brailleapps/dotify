...
<!--		<xsl:value-of select="descendant::text()"/>-->
	...
	<xsl:template name="insertProcessorRenderer">
		...
		<renderer name="table-renderer">
			<rendering-scenario processor="identity" cost="(+ (* 2 $forced-break-count) $total-height (/ (- {$page-width} $min-block-width) {$page-width}))"/>
			<rendering-scenario processor="table-as-block" cost="(+ (* 2 $forced-break-count) $total-height (/ (- {$page-width} $min-block-width) {$page-width}))"/>
		</renderer>
	</xsl:template>
